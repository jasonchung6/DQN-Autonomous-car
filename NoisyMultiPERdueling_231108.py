# CNN 이미지 6장 입력 + 합성곱 레이어 4번
# 신호등 인식 선행 학습: 행동 3가지 (직진, 감속, 가속)
# 모델 파라미터 및 리플레이 버퍼 저장 
...
class Q_network(nn.Module):
    def __init__(self, num_actions):
        ...
        # 상태 가치 레이어 => 특정 상태에서 얻게 되는 보상의 기댓값
        self.state_val_fc = nn.Sequential(
            nn.Linear(1632, 512),
            nn.GELU(),
            #nn.Linear(512, 128),
            nn.Linear(512, 256),
        ...
        self.init_weights(self.state_val_fc)
        self.init_weights(self.advantage_fc)
    ...
    def forward(self, camera, ray, signal):
        ...
        val = self.state_val_fc(x)
        adv = self.advantage_fc(x)
       
        ave_adv = torch.mean(adv, dim=1, keepdim=True)
        # print("val: ",val)
        # print("adv: ", adv)
        # print("ade_adv:",ave_adv)
        Q_values = val + adv - ave_adv
...
class Agent:
    def __init__(self):
        ...
        self.final_epsilon = 0.1  # 231227
        self.epsilon_decay_period = 200000 #231227 #100000 
        self.epsilon_cnt = 0
    ...
    def train(self, step, update_target):
        ...
        try:
            self.obs_camera_list, self.obs_ray_list, self.signal_list, self.action_list, self.reward_list, self.next_obs_camera_list, self.next_obs_ray_list, self.next_signal_list, self.mask_list = zip(*mini_batch)
        except TypeError as e:
            print("Error in unpacking mini_batch:", e)
            # print("total_priority: ", self.memory.tree.total_priority()) => 문제 없음
            print("total_priority: ", self.memory.tree.total_priority())
            print("priority: ", self.memory.priorities)
            print("mini_batch: ", mini_batch)
            print("IS_weight : ", IS_weights) # IS_weight :  [ 0.  0.  0.  0.  0.  0.  0. nan] 이럴 때 오류 발생하는듯?
            print("idxs : ", idxs)
            # 각 리스트별로 오류를 확인
            for idx, data in enumerate(mini_batch):
                try:
                    _ = zip(*data)  # 리스트를 언패킹하여 어떤 부분에서 오류가 나는지 확인
                except TypeError as sub_error:
                    print(f"Error in data at index {idx}:", sub_error)
            raise  # 예외를 다시 발생시켜서 프로그램을 중단하도록 합니다.
...
def main():
    ...
    for epi in range(10001):
        ...
        while True:
            ...
            cur_sample = [obs_camera, obs_ray_tensor, obs_signal, dis_action, reward, next_obs_camera, next_obs_ray_tensor, next_obs_signal, mask]
